...
export default function AdminPage() {
  ...
  const filterList = ['pending', 'approved', 'rejected', 'all']
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-3 rounded-xl mb-4 flex gap-2 flex-wrap border border-slate-200">
          {filterList.map((f) => {
            ...
            const btnClass = isActive
              ? 'px-4 py-2 rounded-lg text-sm font-bold border bg-teal-600 text-white border-teal-600'
              : 'px-4 py-2 rounded-lg text-sm font-bold border bg-white text-teal-600 border-teal-600'
            return (
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {posts.map((post) => {
              const badgeClass = 'text-white text-xs font-bold px-3 py-1 rounded-full ' + statusColor[post.status]
              return (
                <div key={post.id} className="bg-white rounded-xl border border-slate-200 p-5">
                  <div className="flex flex-wrap gap-2 items-center mb-3">
                    <span className={badgeClass}>{statusLabel[post.status]}</span>
                    {post.diseases && post.diseases.categories && (
                    ...
                    <span className="text-sm font-bold text-teal-700 bg-teal-100 px-3 py-1 rounded-full">
                      {post.diseases ? post.diseases.primary_name : ''}
                    </span>
                  ...
                  {post.post_tags && post.post_tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {post.post_tags.map((pt, i) => (
                        <span key={i} className="text-xs bg-slate-50 border px-2 py-0.5 rounded">
                          {pt.tags ? pt.tags.name : ''}
                        </span>
                      ))}
                    </div>
                  )}

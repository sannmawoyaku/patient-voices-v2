...
export default function PostPage() {
  ...
  async function fetchCategories() {
    const result = await supabase.from('categories').select('*').order('display_order')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newPostId = postResult.data[0].id
      ...
      setMessage('投稿ありがとうございます！掲示板に公開されました。')
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans">
      <main className="max-w-3xl mx-auto px-4 py-6">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          {userId && (
            <div className="mx-6 mt-6 bg-teal-50 border border-teal-200 rounded-lg px-4 py-3 text-sm text-slate-700">
              ログイン中のアカウントで投稿します。マイページから後で削除できます。
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map(tag => (
                  <button
                    ...
                    className={formData.tagIds.includes(tag.id) ? 'px-3 py-1.5 rounded-full text-sm font-medium border bg-teal-600 text-white border-teal-600' : 'px-3 py-1.5 rounded-full text-sm font-medium border bg-white text-slate-600 border-slate-300 hover:bg-slate-50'}
                  >
                    {tag.name}
                  </button>
                ))}
            ...
            {message && (
              <div className={(message.includes('失敗') || message.includes('必須') || message.includes('同意')) ? 'p-3 rounded-lg text-sm bg-red-50 text-red-700 border border-red-200' : 'p-3 rounded-lg text-sm bg-green-50 text-green-700 border border-green-200'}>
                {message}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={submitting}
              className={submitting ? 'w-full py-3 rounded-xl font-bold text-lg bg-slate-200 text-slate-400 cursor-not-allowed' : 'w-full py-3 rounded-xl font-bold text-lg bg-teal-600 hover:bg-teal-700 text-white shadow-sm'}
            >
